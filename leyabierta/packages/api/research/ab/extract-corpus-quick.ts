#!/usr/bin/env bun
/**
 * Quick corpus extraction for fast tuning iterations.
 * Samples only 2000 blocks for rapid config testing.
 */

import { Database } from "bun:sqlite";

const DB_PATH = "/Users/alex/00_Programacion/01_Alex/leyabierta/leyabierta/data/leyabierta.db";
const OUT_PATH = "/Users/alex/00_Programacion/01_Alex/leyabierta/leyabierta/data/corpus-preceptos-quick.json";

console.log("Extracting QUICK corpus sample...");
const t0 = Date.now();
...
// Quick sample: 2000 blocks
const QUICK_SAMPLES = 2000;
if (blocks.length > QUICK_SAMPLES) {
  ...
  for (const [normId, count] of Object.entries(normCounts)) {
    const sampleCount = Math.max(2, Math.round((count / total) * QUICK_SAMPLES));
  ...
  blocks.length = 0;
  blocks.push(...sampled);
  console.log(`  Sampled to ${blocks.length} blocks (QUICK)`);
} else {
  console.log(`  No sampling needed (${blocks.length} blocks)`);
}

console.log(`Writing to ${OUT_PATH}...`);
await Bun.write(OUT_PATH, JSON.stringify(blocks));
const size = (await Bun.file(OUT_PATH).size()) / 1024 / 1024;
