/**
 * Prompt (v10), output schema and response parser for the per-article citizen
 * summaries backfill (backfill-citizen-summaries.ts).
 *
 * Kept in its own module, without the script's top-level CLI/DB code, so the
 * exact production prompt and parser can be reused by tests and model
 * evaluations.
 */
