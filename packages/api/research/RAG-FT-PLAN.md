...
| 2026-04-28 | Eval-v2 register skew flagged as a representativeness risk | Heuristic register tagger applied to eval-v2 (205 questions): 88% formal / 6% informal / 5% procedural. Per-register BM25-only baseline (informal n=13, formal n=163, procedural n=11) shows informal R@10=15.4% vs formal R@10=30.1% — informal is BM25's weakest. **Risk**: if we train on a 30/40/30 dataset but eval on a 88/6/5 set, gains on informal are statistically invisible. Mitigation options: (a) expand eval-v2 with ~50 deliberately informal Google-style queries, (b) wait for production `ask_log` data to inform eval mix, (c) accept that Fase 1c per-register R@10 needs caveats about sample size. |
| 2026-04-28 | Mitigation chosen: option (a) — expand eval-v2 via async agents | 3 agents in parallel produced 85 net-new queries (30 informal Google-style, 30 informal natural-Spanish, 25 procedural mixed-style). All 24 distinct expectedNorms verified vigente. Eval-v2 grew 205 → 290. A fourth agent then re-tagged the original 205 with the canonical `register` field, giving full coverage. Heuristic regex tagger deleted (it scored 81% accuracy on the 85 — close, but agent labels are canonical). Final per-register N: informal=96, formal=129, procedural=47. **Canonical baseline (BM25-only, no reranker)**: informal R@10=16.7%, formal R@10=33.3%, procedural R@10=51.1%. Informal is the clear gap — half of formal. That's the slot the reranker has to close. |
| 2026-04-28 | Realistic-retrieval baseline measured (vector + BM25 + RRF, no reranker) | Wired the dumper to call `runRetrievalCore` with a new `skipRerank: true` flag (added to `RunRetrievalCoreOpts` in retrieval.ts). One full pass over 272 in-scope eval-v2 questions. Pre-reranker R@10: informal 78.1%, formal 82.2%, procedural 72.3%, overall 79.0%. **The per-register gap inverts**: informal goes from BM25's worst (16.7%) to comparable with formal (78.1%, +61pp lift) — vector embeddings bridge the citizen-vocabulary gap that BM25 can't. Procedural becomes the new weakest (72.3%) because its literal domain words (modelo 100, certificado de penales) already worked for BM25, so vector's marginal lift is smaller (+21pp). Overall 79% is 8.6pp below the plan's published 87.6% baseline that uses Cohere on top — that 8.6pp is the lift Cohere currently provides, which the FT reranker has to match or beat. |
| 2026-04-28 | Dataset scaled 50 → 300 articles (5 parallel agents, v3 prompt) | Sampled 250 fresh articles (seed=2026), split into 5 shards (3 state ~58 each + 2 ccaa ~38 each), one async agent per shard with v3 prompt. Combined output: 250 articles × ~2.7 q/article = 689 queries. Register split: 33% formal / 39% informal / 28% procedural — within v3's 30/40/30 tolerance band, informal as plurality maintained. 22 traps (~9%), 2 article-number leaks (0.3%, agent slipped past the v3 anti-pattern rule on `BOE-A-2025-76/a2-5` and `BOE-A-2004-4214/a145` — accepted, not enough volume to move metrics). Assembled into `reranker-v3.jsonl`: 689 pairs, 98.8% with both negative types. Combined with the v2 pilot (`reranker-combined.jsonl`): **824 total pairs, 2448 triplets** (informal 973, formal 796, procedural 679). Still well below the 5K target but ~6× the previous pilot — enough to actually train and measure on. |
| 2026-04-29 | First-pass training results — toolchain validates, lift does not yet materialize | Ran four FT experiments on the 824-pair dataset against the realistic baseline (R@10=79.0%) and the Cohere production target (R@10=87.6%). All trained rerankers underperform the no-rerank baseline. Best result: MiniLM-MNR-3ep at R@10=68.4% (still −10.6pp vs no-rerank). The pipeline (train.py + eval.py + dump-eval-candidates.ts) works end-to-end; the gap is real but explainable. **Three findings**: (1) BCE on (query, doc, label∈{0,1}) overfits aggressively — 3ep BCE drops MiniLM to R@10=63.6%, vs 1ep at 68.8%. Train.py was rewritten to use the modern sentence-transformers v5 `CrossEncoderTrainer` with `MultipleNegativesRankingLoss` (in-batch contrastive) which removes the overfitting collapse. (2) Even bge-reranker-v2-m3 *untrained* drops retrieval to R@10=68.0% — base rerankers are tuned on web search and add noise to legal-Spanish RRF rankings. The FT challenge is to undo that bias AND learn legal signals. (3) Memory is a real constraint on M4 Max with concurrent apps — bge-reranker-base at batch 4 seq 128 (forced by OOM at batch 8 seq 256) gives only 3 in-batch negatives, which weakens MNR's contrastive signal and yields R@10=64.3%, *worse* than the smaller MiniLM at batch 16. **Conclusion**: not a plumbing gap, a scale gap. Path forward is dataset to 5K+ pairs (not 824) and either freed memory or smaller-base experiments. Numbers tabulated in `packages/api/research/training/RESULTS.md`. |
