...
// Iteration 6: Hardened system prompt addressing judge feedback:
//   - Explicitly forbid 2nd person (tú/tu/tienes) — cases 5, 22
//   - Explicitly forbid editorial commentary not in source — case 20
//   - Stronger "when empty" rules to reduce empty responses — case 15
//   - Better examples showing correct 3rd person patterns
...
VERIFICACIÓN: [¿uso 3ª persona? ¿añado algo que no está en el artículo?]
</think>
...
Ejemplo 1 (artículo sobre símbolos de Andalucía):
ARTÍCULO: La bandera de Andalucía está formada por tres franjas horizontales, verde, blanca y verde...
RESUMEN: Andalucía tiene como símbolos oficiales la bandera (verde, blanca, verde), el escudo y el himno, aprobados por el Parlamento. El Día de Andalucía se celebra el 28 de febrero.

Ejemplo 2 (artículo sobre plazos de prescripción):
ARTÍCULO: Las infracciones muy graves prescribirán a los tres años, las graves a los dos y las leves a los doce meses...
RESUMEN: Las infracciones por discriminación prescriben en: 3 años (muy graves), 2 años (graves), 1 año (leves). Las sanciones prescriben en los mismos plazos desde que la resolución es ejecutable.

Ejemplo 3 (artículo sobre derechos procesales — mostrar 3ª persona correcta):
ARTÍCULO: La defensa de una persona investigada podrá solicitar que se practiquen diligencias de investigación...
RESUMEN: La persona investigada puede solicitar diligencias. El Fiscal Europeo las acordará si son relevantes. Si las deniega, se pueden impugnar ante el Juez de Garantías.`;
...
async function main() {
	...
	// Retry wrapper: if Qwen fails, retry up to 2 times.
	// Rate-limit (429) bans last ~1 minute → 65s wait.
	// 524s (gateway timeout) are intermittent → 2s wait, try again.
	async function callQwenWithRetry(row: SampleRow): Promise<ProviderResult> {
		...
		if (result.error) {
			...
			console.log(`  Case ${idx}: Qwen error (${result.error.slice(0, 60)}), retrying${isRateLimit ? " (65s wait)" : ""}…`);
			if (isRateLimit) {
				await new Promise((r) => setTimeout(r, 65_000));
			}
			const result2 = await callChat(qwen, SYSTEM_PROMPT, userPromptFor(row), 180_000);
			if (result2.error) {
				// Second retry for 524s only (rate limits already cleared by 65s wait)
				if (is524) {
					console.log(`  Case ${idx}: Retry 2 failed (${result2.error.slice(0, 40)}), trying once more…`);
