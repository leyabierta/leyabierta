...
- `GET /v1/reforms/personal` — reforms filtered by materias + jurisdiction (for /cambios/para-mi/; jurisdiction via `j`, `jurisdiccion` or its alias `jurisdiction`, default `es`)
...
a failure alerts and the run continues to OG images and the index rebuild.
The `leyes` repo push (Step 9.6) runs LAST — after ingest, ingest-analisis, the
...
Email alerts (Resend, subscriptions, per-law follows) were removed on 2026-09-24:
the site collects no email addresses and has no accounts. People follow changes with
the RSS feeds (`/feed.xml`, `/v1/feed.xml`, `/v1/feed-omnibus.xml`), the per-law Atom
feed of the `leyes` repo on GitHub (linked from each law page), and `/cambios/para-mi/`,
whose preferences live only in the visitor's browser (`localStorage`). Analytics are
Umami only (self-hosted, no cookies). Do not reintroduce email or accounts without
updating `/privacidad/` first.
...
- `/cambios/para-mi/` — personal legislative changelog (client-side, filtered by the visitor's materias; `/mis-cambios` redirects here)
- `/cambios` — public changelog of all recent reforms (client-side)
- `/reforma` — single reform detail page
- `/mi-situacion` — preference wizard for `/cambios/para-mi/` (stored only in `localStorage`; `/alertas*` redirects to `/cambios/para-mi/`)
...
- This applies to: page content, headings, meta descriptions, aria-labels, form labels, error messages, and any text visible to citizens.
