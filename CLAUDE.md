...
| `leyes-es` (separate) | Spanish legislation as Markdown + Git history |

The `leyes-es` repo is generated by the pipeline. Each file is a norm, each commit is a reform. Future countries get their own repo (`leyes-fr`, `leyes-de`, etc.).
...
├── output/es/         # Local clone of leyes-es (gitignored)
├── biome.json
├── package.json
└── tsconfig.json
```

### Output repo structure (leyes-es)
